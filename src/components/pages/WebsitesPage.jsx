    ...
    <div className="bg-white min-h-screen text-foreground antialiased selection:bg-black/30 selection:text-black">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.7 }}>
            ...
            <div className="rounded-2xl overflow-hidden shadow-2xl border border-black/8">
              ...
              <div className="bg-gray-50 border-b border-black/8 px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 bg-white rounded-md px-3 py-1 text-xs text-black/40 font-body border border-black/8">
              ...
              <div className="bg-gradient-to-br from-slate-50 to-white h-72 md:h-96 relative overflow-hidden flex items-center justify-center">
                ...
                {[
                  ...
                  <motion.div key={i}
                    ...
                    className="bg-white rounded-xl shadow-md border border-black/5"
                  />
                ))}
                <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.8 }}
                  className="relative z-10 text-center">
                  <span className="text-4xl font-heading italic text-black/20">Ihr nächstes Projekt</span>
                </motion.div>
              </div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-3 gap-6 mt-10">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="text-4xl md:text-5xl font-heading italic text-black mb-1">{m.val}</div>
                <div className="text-black/50 font-body text-sm">{m.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            <div className="liquid-glass rounded-full px-4 py-1.5 mb-6">
              <span className="text-black text-xs font-medium font-body uppercase tracking-wider">Was wir bauen</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-black">Für jede Branche &amp; jedes Ziel.</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {showcase.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <span className="font-heading italic text-black text-xl leading-tight">{item.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            <div className="liquid-glass rounded-full px-4 py-1.5 mb-6">
              <span className="text-black text-xs font-medium font-body uppercase tracking-wider">Unser Prozess</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-black">Von der Idee zum Launch.</h2>
          </div>
            ...
            {process.map((step, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12 }}
                ...
                <div className="flex items-center justify-between">
                  <span className="font-heading italic text-5xl text-black/10">{step.step}</span>
                ...
                <h3 className="text-lg font-heading italic text-black">{step.title}</h3>
                <p className="text-black/60 font-body font-light text-sm leading-relaxed">{step.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-black">Alles, was eine Top-Website braucht.</h2>
          </div>
