    ...
    <div className="bg-white min-h-screen text-foreground antialiased selection:bg-black/30 selection:text-[#1e294d]">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            <div className="liquid-glass rounded-full px-4 py-1.5 mb-6">
              <span className="text-[#1e294d] text-xs font-medium font-body uppercase tracking-wider">Visualisierung</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-[#1e294d]">So sieht Ihr Workflow aus.</h2>
          </div>
              ...
              {workflowNodes.map((node, i) => (
                <motion.div key={i}
                  ...
                  <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center shadow-md text-white text-sm font-body font-semibold
                    ${node.type === 'start' ? 'bg-[#213979]' : node.type === 'ai' ? 'bg-gradient-to-br from-violet-500 to-purple-700' : node.type === 'end' ? 'bg-green-600' : 'bg-gray-700'}`}>
                    {node.type === 'ai' ? <Bot className="w-5 h-5" /> : node.type === 'start' ? <Zap className="w-5 h-5" /> : node.type === 'end' ? '✓' : <Settings className="w-5 h-5" />}
                  </div>
                  <span className="text-xs font-body text-[#1e294d]/60 whitespace-nowrap">{node.label}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-3 gap-6 mt-10">
            {savings.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="text-4xl md:text-5xl font-heading italic text-[#1e294d] mb-1">{s.val}</div>
                <div className="text-[#1e294d]/50 font-body text-sm">{s.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            <div className="liquid-glass rounded-full px-4 py-1.5 mb-6">
              <span className="text-[#1e294d] text-xs font-medium font-body uppercase tracking-wider">Anwendungsfälle</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-[#1e294d]">Was wir automatisieren.</h2>
          </div>
            ...
            {useCases.map((uc, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="liquid-glass rounded-full w-12 h-12 flex items-center justify-center shrink-0">
                  <uc.icon className="w-5 h-5 text-[#1e294d]" />
                </div>
                <div>
                  <h3 className="font-heading italic text-xl text-[#1e294d] mb-2">{uc.title}</h3>
                  <p className="text-[#1e294d]/60 font-body font-light text-sm leading-relaxed">{uc.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-[#1e294d]">Technologie, die liefert.</h2>
          </div>
