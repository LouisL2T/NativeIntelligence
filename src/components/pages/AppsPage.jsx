      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-16">
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}
            ...
            {/* Phone frame */}
            <div className="relative w-64 h-[520px] bg-black rounded-[3rem] shadow-2xl border-[8px] border-black overflow-hidden">
              {/* Screen */}
              <div className="absolute inset-0 bg-gradient-to-br from-slate-50 to-white overflow-hidden">
                {/* Status bar */}
                <div className="h-8 bg-white/80 flex items-center justify-between px-6 text-[9px] font-body text-black/40">
                ...
                {/* Bottom nav */}
                <div className="absolute bottom-0 left-0 right-0 h-16 bg-white border-t border-black/5 flex items-center justify-around px-4">
                  {[0,1,2,3].map(i => (
                    <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.9 + i * 0.1 }}
                      className={`w-8 h-8 rounded-full ${i === 1 ? 'bg-black' : 'bg-black/10'}`} />
                  ))}
                </div>
              </div>
              {/* Notch */}
          ...
          <div className="flex-1">
            <div className="liquid-glass rounded-full px-4 py-1.5 mb-6 inline-block">
              <span className="text-black text-xs font-medium font-body uppercase tracking-wider">Was uns unterscheidet</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-heading italic text-black leading-[0.9] mb-8">
            ...
            <div className="flex flex-col gap-5">
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  ...
                  <span className="font-body text-black/70 text-base">{point}</span>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            <div className="liquid-glass rounded-full px-4 py-1.5 mb-6">
              <span className="text-black text-xs font-medium font-body uppercase tracking-wider">Plattformen</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-black">Überall zu Hause.</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {platforms.map((p, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <p.icon className="w-6 h-6 text-black/60" />
                <span className="font-heading italic text-black text-xl leading-tight">{p.name}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-black">Vom Konzept zum Store.</h2>
          </div>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-heading italic tracking-tight leading-[0.9] text-black">Enterprise-ready, Day One.</h2>
          </div>
